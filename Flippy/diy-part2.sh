...
sed -i "s/${orig_version}/R${date_version} by ZHDeveloper/g" package/lean/default-settings/files/zzz-default-settings
...
# sed -i 's/"PassWall 2"/"HelloWall"/g' `grep "PassWall 2" -rl ./`
# sed -i 's/"TTYD 终端"/"终端"/g' `grep "TTYD 终端" -rl ./`
# sed -i 's/"KMS 服务器"/"KMS激活"/g' `grep "KMS 服务器" -rl ./`
# sed -i 's/"Alist 文件列表"/"Alist网盘"/g' `grep "Alist 文件列表" -rl ./`
# sed -i 's/"Turbo ACC 网络加速"/"网络加速"/g' `grep "Turbo ACC 网络加速" -rl ./`
# sed -i 's/"ShadowSocksR Plus+"/"ShadowSocksR"/g' `grep "ShadowSocksR Plus+" -rl ./`
